...
namespace freetype {
	...
	void make_dlist(FT_Face face, char ch, GLuint list_base, GLuint * tex_base) {
		...
		{
			std::cerr << "Error: FT_Load_Glyph failed\n";
		...
		{
			std::cerr << "Error: FT_Get_Glyph failed\n";
		...
		glTranslatef(0, static_cast<float>(bitmap_glyph->top) - static_cast<float>(bitmap.rows), 0);
	...
	void font_data::init(const char * fname, unsigned int h) {
		...
		{
			std::cerr << "Error: FT_New_Face failed (there is probably a problem with your font file)\n";
	...
	float print(const font_data& ft_font, float x, float y, const char* fmt, ...) {
		...
		*/
		...
		coordVec[0] = (coordVec[0] - projectionMatScreen[3][0]) / projectionMatScreen[0][0];
		coordVec[1] = (coordVec[1] - projectionMatScreen[3][1]) / projectionMatScreen[1][1];

		x = coordVec[0];
		y = coordVec[1];

		//std::cout << 1.0/std::abs(480 * projectionMatScreen[1][1] + projectionMatScreen[3][1]) << "\n";

		//std::cout << ((1 - projectionMatScreen[3][1]) / projectionMatScreen[1][1])/480.0f << "\n\n";

		GLuint font = ft_font.list_base;
		float h = ft_font.h / .63f;						//We make the height about 1.5* that of

		//h *= ((1 - projectionMatScreen[3][1]) / projectionMatScreen[1][1]) / 480.0f;
		...
		//convert the postion to the coordinates I use
		len = len * projectionMatScreen[0][0] + projectionMatScreen[3][0];
		len = (len - projectionMatMine[3][0]) / projectionMatMine[0][0];
...
//		std::cout << fmt << "\n";
		...
		*/
		...
		glColor4f(0, 0, 0, 0); //makes what im printing invisible cos I don't know how this function works
